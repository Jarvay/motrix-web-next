...
async fn get_system_config(
    ...
    let value: Value =
        serde_json::from_str(&content).map_err(|e| app_err(AppError::Store(e.to_string())))?;
    Ok(Json(value))
}
...
async fn factory_reset(State(state): State<AppState>) -> Result<Json<()>, (StatusCode, String)> {
