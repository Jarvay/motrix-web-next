...
async fn get_task_with_peers(
    ...
    let task = state.aria2_client.tell_status(&gid).await.map_err(app_err)?;
    ...
    let mut result =
        serde_json::to_value(&task).map_err(|e| AppError::Aria2(format!("serialize task: {e}"))).map_err(app_err)?;
    result["peers"] = peers;
    Ok(axum::Json(result))
}
...
async fn batch_force_pause(
    ...
    let calls: Vec<_> = body.gids.iter().map(|gid| {
        ("forcePause".to_string(), vec![serde_json::json!(gid)])
    }).collect();
...
async fn batch_unpause(
    ...
    let calls: Vec<_> = body.gids.iter().map(|gid| {
        ("unpause".to_string(), vec![serde_json::json!(gid)])
    }).collect();
    map_err(state.aria2_client.multicall(calls).await)
}
...
async fn batch_force_remove(
    ...
    let calls: Vec<_> = body.gids.iter().map(|gid| {
        ("forceRemove".to_string(), vec![serde_json::json!(gid)])
    }).collect();
    map_err(state.aria2_client.multicall(calls).await)
}
...
async fn ed2k_search(
    ...
    map_err(state.aria2_client.ed2k_search(&body.query, serde_json::json!({})).await)
}
...
async fn get_task_files(
    ...
    let value = serde_json::to_value(&files)
        .map_err(|e| AppError::Aria2(format!("serialize files: {e}"))).map_err(app_err)?;
    Ok(axum::Json(value))
}
...
async fn purge_download_result(
    State(state): State<AppState>,
) -> AppResult<String> {
...
async fn get_global_stat(State(state): State<AppState>) -> AppResult<Value> {
    let stat = state.aria2_client.get_global_stat().await.map_err(app_err)?;
    let value = serde_json::to_value(&stat)
        .map_err(|e| AppError::Aria2(format!("serialize stat: {e}"))).map_err(app_err)?;
    Ok(axum::Json(value))
}
